...
public class WallpaperCommandBuilder() : FileCommandBuilder("Wallpaper Command", "Wallpaper File") {
    public override Command? BuildCommand(Panel inputPanel) {
        ...
        else // implies URL input
        {
            ...
            else if (!Utils.IsImageFile(content)) { // I know the "else" here is redundant, but it emphasizes that these two clauses are mutually exclusive
                MessageBox.Show("File format not supported. Currently supported formats are JPG, JPEG and PNG.");
                return null;
            }
            upperTextBox.Clear();
        }
        return new WallpaperCommand(content);
    }
}